...
test('Wave 8 focused stress stays bounded, idempotent, revision-safe and non-authoritative',async()=>{
  ...
  const m=core.metricsSnapshot();assert.equal(decided,700);assert.equal(skipped,400);assert.equal(abstained,200);assert.equal(stale,200);assert.equal(late,100);assert.equal(authorityViolations,0);assert.equal(replayMismatch,0);
