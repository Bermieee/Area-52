...
// D
test('PRESERVE_MULTIPLE keeps compatible alternatives explicit without forcing a winner',async()=>{const core=new JevDecisionCore({providerExecutor:provider(()=>({payload:out({outcome:JevOutcome.UNRESOLVED,decisionCode:JevDecisionShape.PRESERVE_MULTIPLE,selectedOptionIds:['A','B'],rejectedOptionIds:[],evidenceUsed:['eA','eB'],unresolvedFactors:['both alternatives remain supportable'],confidence:.55})}))});const x=await core.decide(request({decisionId:'PM',decisionShape:JevDecisionShape.PRESERVE_MULTIPLE}),{currentRevisionState:current});assert.equal(x.outcome,'UNRESOLVED');assert.equal(x.decisionCode,'PRESERVE_MULTIPLE');assert.deepEqual(x.selectedOptionIds,['A','B']);});

test('CLASSIFY_RELATIONSHIP uses the shared bounded kernel and one explicit option',async()=>{const core=new JevDecisionCore({providerExecutor:provider(()=>({payload:out({decisionCode:JevDecisionShape.CLASSIFY_RELATIONSHIP,classification:'COMPLEMENTARY',selectedOptionIds:['A'],rejectedOptionIds:['B'],evidenceUsed:['eA']})}))});const x=await core.decide(request({decisionId:'CLASSIFY',decisionShape:JevDecisionShape.CLASSIFY_RELATIONSHIP,allowedOutcomes:[JevDecisionShape.CLASSIFY_RELATIONSHIP,JevDecisionShape.UNRESOLVED,JevDecisionShape.ABSTAIN]}),{currentRevisionState:current});assert.equal(x.outcome,'DECIDED');assert.equal(x.classification,'COMPLEMENTARY');assert.deepEqual(x.selectedOptionIds,['A']);});

test('RANK_BOUNDED_OPTIONS retains only supplied option identities in bounded order',async()=>{const core=new JevDecisionCore({providerExecutor:provider(()=>({payload:out({decisionCode:JevDecisionShape.RANK_BOUNDED_OPTIONS,selectedOptionIds:['B','A'],rejectedOptionIds:[],evidenceUsed:['eA','eB']})}))});const x=await core.decide(request({decisionId:'RANK',decisionShape:JevDecisionShape.RANK_BOUNDED_OPTIONS,allowedOutcomes:[JevDecisionShape.RANK_BOUNDED_OPTIONS,JevDecisionShape.UNRESOLVED,JevDecisionShape.ABSTAIN]}),{currentRevisionState:current});assert.equal(x.outcome,'DECIDED');assert.deepEqual(x.selectedOptionIds,['B','A']);});
