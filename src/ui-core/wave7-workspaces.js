...
function renderGenerationInspector(object,{document:d,scope,services},forensics,promptPlan){
  ...
  if(level!==ProductDetailLevel.NORMAL){
    ...
    if(path){root.append(element(d,'h3',{text:'Decision trail'}));for(const step of path.steps){const row=element(d,'div',{className:'a52-inspector-trace-row'});row.append(makeBadge(d,step.status,statusToken(step.status)),element(d,'span',{text:`${step.label} — ${step.impact}`}));if(step.status!=='MISSING')row.append(createAuthorityPill(d,step.authority?.authority??step.authority));root.append(row);}if(path.lateAfterSeal.length)root.append(state(d,'Late work stayed outside the sealed generation',`${path.lateAfterSeal.length} late result${path.lateAfterSeal.length===1?' is':'s are'} recorded after the Context Seal and did not rewrite it.`,'warning'));}
  }
  if(level===ProductDetailLevel.ADVANCED){root.append(element(d,'h3',{text:'Advanced read-only payload'}),element(d,'pre',{className:'a52-context-packet',text:JSON.stringify(x,null,2)}));}
  return root;
}
...
function renderForensicInspector(object,{document:d,scope,services},forensics){
  ...
  if(item.rawPayloadAvailable&&forensics&&level===ProductDetailLevel.ADVANCED){const out=element(d,'div',{className:'a52-lazy-detail'}),button=createButton(d,{label:'Load raw diagnostic detail',scope,variant:'quiet',onPress:async()=>{button.disabled=true;try{const payload=await forensics.loadDetail(item.id);out.replaceChildren(payload==null?state(d,'Detail unavailable','The owning producer did not return a retained payload.','offline'):element(d,'pre',{className:'a52-context-packet',text:JSON.stringify(payload,null,2)}));}catch(error){out.replaceChildren(state(d,'Detail failed',String(error?.message??error),'warning'));}}});root.append(button,out);}else if(item.rawPayloadAvailable)root.append(element(d,'p',{className:'a52-muted',text:'Raw diagnostic detail is available in Advanced view only.'}));
  return root;
}
function forensicPathCard(d,path,ctx){
  const card=element(d,'section',{className:'a52-card a52-forensic-path',attrs:{'aria-label':'Generation cognitive path'}});card.append(element(d,'h2',{text:'Generation path'}),element(d,'p',{className:'a52-muted',text:'Recorded semantic steps only. Missing steps stay missing; Runtime work remains a separate cross-link.'}));
  const steps=element(d,'div',{className:'a52-forensic-path__steps',attrs:{role:'list'}});
  for(const step of path.steps){const node=element(d,'article',{className:'a52-forensic-path__step',attrs:{role:'listitem','aria-label':`${step.label}: ${step.status}`},dataset:{status:step.status}}),head=element(d,'div',{className:'a52-inline-status'});head.append(makeBadge(d,step.status,statusToken(step.status)),element(d,'strong',{text:step.label}));if(step.status!=='MISSING')head.append(createAuthorityPill(d,step.authority?.authority??step.authority));node.append(head,element(d,'p',{className:'a52-muted',text:step.impact}));steps.append(node);}card.append(steps);
