...
export class ContextDeliveryEngine extends PromptPlanner{
  ...
  deliver(input){const planned=this.createPlan(input);if(!planned.ok)return planned;const rendered=this.render({plan:planned.plan,profile:planned.profile});if(!rendered.ok)return{...planned,...rendered,ok:false};return{...planned,...rendered,ok:true,status:DeliveryStatus.READY};}
}
