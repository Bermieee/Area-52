...
export function exportSceneLifecycleState({registry,stack,episodeCompiler,graph,prefetchTrigger=null,narrativeFeed=null,transitionManager=null,contextInvalidationPublisher=null}){
  return clone({kind:'SceneLifecycleState',version:1,registry:registry.exportState(),stack:stack.exportState(),episodes:episodeCompiler.exportState(),graph:graph.exportState(),prefetch:prefetchTrigger?.exportState()??null,narrativeFeed:narrativeFeed?.exportState()??null,transitions:transitionManager?.exportState()??null,contextInvalidations:contextInvalidationPublisher?.exportState()??null});
}
