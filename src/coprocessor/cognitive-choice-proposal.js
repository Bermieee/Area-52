...
export function buildCoprocessorChoiceProposal({fanOutPlan,turnEvent,plannerInput={},roleCatalog=[],capabilityProfiles=[],ownerSignals={},policyVersion='sidecar-choice-v1',telemetry=null,limits={}}={}){
  ...
  for(const role of roleCatalog){
    ...
    if(task){disposition=task.resultClass===ResultClass.DEFERRED?CoprocessorChoiceDisposition.DEFERRED:CoprocessorChoiceDisposition.NOMINATED;reasons=[...(nom?.reasonCodes??[]),...need.reasons];}
    else if(avail.unavailable){disposition=CoprocessorChoiceDisposition.UNAVAILABLE;reasons=avail.reasons;}
...
function availability(role,p,i){const caps=role.requiredCapabilities??[],matching=p.filter(x=>caps.every(c=>x.capabilities.includes(c))),reasons=[],explicit=i.availableCapabilities==null?null:new Set(i.availableCapabilities);
  ...
  const eligible=matching.filter(x=>x.available&&['HEALTHY','DEGRADED'].includes(x.health)&&x.currentLoad<x.capacity);if(matching.length&&!eligible.length){if(matching.some(x=>x.currentLoad>=x.capacity))reasons.push(CoprocessorChoiceReason.PROVIDER_OVERLOADED);if(matching.every(x=>!x.available))reasons.push(CoprocessorChoiceReason.PROVIDER_UNAVAILABLE);}
...
function jevQuestion(v){if(!v||typeof v!=='object')return{questionId:null,decisionShape:null,optionIds:[],evidenceRefs:[],questionHash:null,rawQuestionRetained:false};const raw=typeof v.query==='string'?v.query:typeof v.question==='string'?v.question:null;
  return{questionId:v.questionId??v.requestId??v.decisionId??null,decisionShape:v.decisionShape??null,optionIds:[...new Set(v.optionIds??(v.options??[]).map(x=>x.optionId).filter(Boolean))].slice(0,32),evidenceRefs:[...new Set((v.evidenceRefs??[]).map(x=>typeof x==='string'?x:x.evidenceId??x.ref).filter(Boolean))].slice(0,64),questionHash:raw?sha256Hex(raw).slice(0,24):null,rawQuestionRetained:false};}
