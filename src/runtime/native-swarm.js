...
export class NativeTurnRuntime {
  ...
  #submitJob(job, turn) {
    ...
    const obligation = {
      ...
      resourceLimits: structuredClone(job.resourceLimits ?? job.metadata?.resourceLimits ?? {}),
      batchHint: { maxSliceUnits: 1, ...(job.batchHint ?? {}) },
      checkpointPolicy: { maxUnitsPerCheckpoint: 1, ...(job.checkpointPolicy ?? {}) },
      yieldPolicy: structuredClone(job.yieldPolicy ?? job.metadata?.yieldPolicy ?? {}),
      dedupeKey: `native:${turn.dedupeKey}:${job.dedupeKey}`,
      conflictKey: job.conflictKey ?? job.metadata?.conflictKey ?? null,
