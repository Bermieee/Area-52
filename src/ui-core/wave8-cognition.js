...
export function normalizeCognitiveChoiceReceipt(receipt){
  ...
  return deepFreeze({
    ...
    jevDecision,precisionDecision,retrievalDecision,abstentions:safeArray(receipt.abstentions),generationEvidenceRefs:safeArray(receipt.generationEvidenceRefs??receipt.finalEvidenceRefs),
...
export function buildLiveCognitionPath({
  ...
  stages.push(stageScatter(scatter,modes.scatter));
  stages.push(stageSensory(sensory,modes.sensory));
  stages.push(stageRetrieval(truth,corrective,modes.truth));
  stages.push(stageTruth(truth,modes.truth));
  stages.push(stageJev(jev,modes.jev));
  stages.push(stagePrecision(precision,modes.precision));
  stages.push(stageGather(gather,modes.gather));
  ...
  const source=createProductSourceStatus({
    mode:available.length?(degraded?ProductDataMode.DEGRADED:ProductDataMode.LIVE):ProductDataMode.UNAVAILABLE,
...
function stageGather(x,mode){if(!x)return createStage({id:CognitionStageId.GATHER,label:'Gather',state:CognitionStageState.UNAVAILABLE,summary:'Gather receipt unavailable.'});return createStage({id:CognitionStageId.GATHER,label:'Gather',state:x.state,summary:`${x.counts.ADMITTED} admitted · ${x.counts.STALE} stale · ${x.counts.LATE} late · ${x.counts.REJECTED+x.counts.INVALID} rejected/invalid`,reason:x.reason,receiptRef:x.receiptId,details:{mode}});}
