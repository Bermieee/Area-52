...
export const CORE_SLOT_SPECS={
  ...
  [PromptSlot.CURRENT_SCENE]:{owner:'CONTEXT_COMPILER',allowedSources:[ContributionSource.SEALED_PACKET],role:'context',protected:true,semantic:true,band:DeliveryBand.REVISIONED_MIDDLE,cacheEligible:true},
...
export class ModelProfileRegistry{
  constructor({profiles=[]}={}){
    ...
    this.register(createModelProfile({modelProfileId:'GENERIC_SAFE',revision:'1',contextWindow:4096,reservedTokens:256,structuredContextPreference:'COMPACT',positionOrder:DEFAULT_ORDER,segmentStrategy:'SLOT_ATOMIC',bandBySlot:Object.fromEntries(DEFAULT_ORDER.map(slot=>[slot,CORE_SLOT_SPECS[slot].band])),cacheCharacteristics:{stablePrefix:false,routeDependent:true,assumption:'CONSERVATIVE_UNKNOWN'},adapterId:'messages-v1',deliveryPolicyRevision:'1',fallbackProfileId:'CACHE_STABLE'}));
    this.register(createModelProfile({modelProfileId:'OPENROUTER_CONSERVATIVE',revision:'1',contextWindow:4096,reservedTokens:256,structuredContextPreference:'COMPACT',positionOrder:DEFAULT_ORDER,segmentStrategy:'BAND_GROUPED',bandBySlot:Object.fromEntries(DEFAULT_ORDER.map(slot=>[slot,CORE_SLOT_SPECS[slot].band])),cacheCharacteristics:{stablePrefix:false,routeDependent:true,assumption:'UNMEASURED_ROUTE'},adapterId:'messages-v1',deliveryPolicyRevision:'1',fallbackProfileId:'GENERIC_SAFE'}));
