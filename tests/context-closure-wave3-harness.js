...
export function runContextClosureWave3GoldenWorld(){
  ...
  const after=JSON.stringify(core.currentWorldModel()),threadPriority=published.packet.semanticPriority.filter(x=>x.reasons.some(r=>r==='THREAD_RELEVANCE:find-sun-blade')),profile=core.delivery.profileRegistry.get('RECENCY_WEIGHTED'),newOrder=[PromptSlot.SYSTEM_POLICY,PromptSlot.WORLD_FOUNDATION,PromptSlot.CHARACTER_FOUNDATION,PromptSlot.HISTORICAL_SUPPORT,PromptSlot.RELEVANT_LORE,PromptSlot.EPISODIC_MEMORY,PromptSlot.UNRESOLVED_EVIDENCE,PromptSlot.ACTIVE_THREADS,PromptSlot.CURRENT_WORLD_STATE,PromptSlot.CURRENT_CHARACTER_STATE,PromptSlot.CURRENT_SCENE,PromptSlot.RECENT_NARRATIVE,PromptSlot.USER_INPUT];
  ...
  const metrics={
    ...
    neutralSizingPresent:published.packet.semanticSizing?.serializedBytes>0&&published.packet.semanticSizing?.tokenEstimate===null,
    ...
    activeThreadCannotCreateTruth:before===after&&!falseCurrent,staleThreadRejected:staleThread.packet.activeThreads.length===0&&staleThread.packet.threadDiagnostics.staleThreadIds.includes('find-sun-blade'),
    ...
    compilerPacketModelIndependent:published.packet.semanticSizing.modelProfileId===null&&!('estimatedModelTokens' in published.packet),
  };
