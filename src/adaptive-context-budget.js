...
export class AdaptiveBudgetAllocator{
  ...
  allocate({sections,profile,intent='DEFAULT',userInput='',budgetTokens=null}){
    ...
    const bySlot=new Map(decisions.map(x=>[x.slot,x])),ordered=sections.map(x=>bySlot.get(x.slot)).filter(Boolean),dropped=ordered.filter(x=>x.representation===RepresentationMode.OMITTED).map(x=>({slot:x.slot,reason:'OPTIONAL_BUDGET_PRESSURE'})),fallbackDecisions=[];
    if(ordered.some(x=>x.representation===RepresentationMode.COMPACT&&profile.structuredContextPreference==='RICH'))fallbackDecisions.push('COMPACT_SAFE_REPRESENTATION');if(dropped.length)fallbackDecisions.push('DROP_OPTIONAL_MATERIAL');const allocated=ordered.reduce((sum,x)=>sum+x.allocatedTokens,0);
