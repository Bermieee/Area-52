...
export function createPhase2ReviewController({document:doc=globalThis.document,storage=globalThis.localStorage}={}){
  ...
  controls.scenario?.addEventListener('change',e=>mountScenario(e.target.value));controls.detail?.addEventListener('change',e=>setDetail(e.target.value));controls.layout?.addEventListener('change',e=>setLayout(e.target.value));controls.width?.addEventListener('change',e=>setDock(e.target.value));controls.inspector?.addEventListener('change',e=>setInspector(e.target.checked));controls.reset?.addEventListener('click',()=>{stateStore.clear();controls.scenario.value='healthy';controls.detail.value='NORMAL';controls.layout.value='desktop';controls.width.value='w720';controls.inspector.checked=false;mountScenario('healthy');});
  doc.addEventListener('keydown',e=>{if(e.key==='Escape'&&ui?.presentation.get().inspectorVisible){ui.presentation.setInspector(false);if(controls.inspector)controls.inspector.checked=false;}});mountScenario(currentScenario);
  return{get ui(){return ui;},scenarios,stateStore,mountScenario,setLayout,setDock,setDetail,setInspector,destroy(){ui?.destroy?.();ui=null;}};
}
