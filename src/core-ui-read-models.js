...
export function createPromptPlanReadModel(plan,{integrityReceipt=null}={}){
  ...
  const deferredBySlot=new Map((plan.deferred??[]).map(row=>[String(row.slot),row])),droppedBySlot=new Map((plan.dropped??[]).map(row=>[String(row.slot),row]));
  const slotAllocation=(plan.sections??[]).map(s=>{const deferred=deferredBySlot.get(String(s.slot))??null,dropped=droppedBySlot.get(String(s.slot))??null,omission=deferred??dropped,omitted=s.representation==='OMITTED'||Boolean(omission);return{slot:s.slot,state:omitted?(deferred?'DEFERRED':'DROPPED'):'INCLUDED',included:!omitted,representation:s.representation??null,allocatedTokens:Number.isFinite(Number(s.allocatedTokens))?Number(s.allocatedTokens):null,minimumTokens:Number.isFinite(Number(s.compactTokens))?Number(s.compactTokens):null,richTokens:Number.isFinite(Number(s.richTokens))?Number(s.richTokens):null,targetTokens:Number.isFinite(Number(s.targetTokens))?Number(s.targetTokens):null,estimatedTokens:Number.isFinite(Number(s.allocatedTokens))?Number(s.allocatedTokens):(s.estimatedTokens??s.tokenEstimate??null),required:Boolean(s.required),protected:Boolean(s.protected),reason:omission?.reason??null,requiredTokens:omission?.requiredTokens??(omitted&&Number.isFinite(Number(s.compactTokens))?Number(s.compactTokens):null),remainingTokensAtDecision:omission?.remainingTokensAtDecision??null,shortfallTokens:omission?.shortfallTokens??null};});
  return frozen({kind:'PromptPlanReadModel',contractVersion:'1.1.0',promptPlanId:plan.promptPlanId,generationId:plan.generationId,turnId:plan.turnId,contextSealId:plan.contextSealId,sealedPacketHash:plan.sealedPacketHash,modelProfileId:plan.modelProfileId,modelProfileRevision:plan.modelProfileRevision,deliveryPolicyRevision:plan.deliveryPolicyRevision,worldRevision:plan.worldRevision,sceneId:plan.diagnosticReceipt?.sceneIntegration?.sceneId??null,sceneRevision:plan.sceneRevision,sceneIntegration:clone(plan.diagnosticReceipt?.sceneIntegration??null),sourceRevisionRefs:uniq(plan.sourceRevisionDependencies),
    slotAllocation,sectionOrder:[...(plan.ordering??[])],reuseDecisions:clone(plan.reuseDecisions??[]),cacheDecisions:clone(plan.cacheDecisions??[]),reusedSegments:reused,rebuiltSegments:rebuilt,dropped:clone(plan.dropped??[]),deferred:clone(plan.deferred??[]),fallbackDecisions:clone(plan.fallbackDecisions??[]),representationDensity:(plan.sections??[]).map(s=>({slot:s.slot,representation:s.representation??null})),budget:clone(plan.budget??{}),estimatedTokens:plan.budget?.estimatedTokens??plan.budget?.usedTokens??plan.budget?.allocated??null,integrityStatus:integrityReceipt?.valid===false?'ERROR':plan.status==='READY'?'READY':plan.status,health:healthFrom({error:integrityReceipt?.valid===false,degraded:(plan.dropped?.length??0)>0||(plan.deferred?.length??0)>0,reasons:[...((plan.dropped?.length??0)?['CONTENT_DROPPED']:[]),...((plan.deferred?.length??0)?['CONTENT_DEFERRED']:[])]}),authority:'READ_ONLY',mutationAuthority:false});
}
