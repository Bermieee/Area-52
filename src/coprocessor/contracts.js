...
export function createTurnEnvelope(input = {}) {
  ...
  return deepFreeze({
    ...
    eventVersion: requiredString(input.eventVersion ?? COPROCESSOR_SCHEMA_VERSION, 'eventVersion'),
...
export function createCognitiveTask(input = {}) {
  ...
  return deepFreeze({
    ...
    optionalCapabilities: uniqueStrings(input.optionalCapabilities ?? [], 'optionalCapabilities'),
    fallbackCapabilities: uniqueStrings(input.fallbackCapabilities ?? [], 'fallbackCapabilities'),
    capabilityRequests: normalizeCapabilityRequests(input.capabilityRequests ?? requiredCapabilities),
    fallbackCapabilitySets: normalizeFallbackCapabilitySets(input.fallbackCapabilitySets ?? (input.fallbackCapabilities?.length ? [input.fallbackCapabilities] : [])),
    cognitiveLayer: requiredString(input.cognitiveLayer ?? 'L1', 'cognitiveLayer'),
    resultClass,
...
export function createWorkerResult(input = {}) {
  ...
  return deepFreeze({
    ...
    modelId: input.modelId == null ? null : requiredString(input.modelId, 'modelId'),
    ...
    intentFingerprint: input.intentFingerprint == null ? null : requiredString(input.intentFingerprint, 'intentFingerprint'),
    providerMetadata: cloneSerializable(input.providerMetadata ?? {}, 'providerMetadata'),
    startedAt,
    completedAt,
