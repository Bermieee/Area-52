...
export class NarrativeFeedAdapter{
  ...
  sourceRevisionIdFor({chatId,messageId,messageRevision,swipeId=null}){return stable('chat',chatId,'message',messageId,'r',messageRevision,swipeId??'primary');}
  findSourceRevision(chatId,sourceRevisionId){const chat=this.chats.get(chatId);if(!chat)return null;for(const m of chat.messages.values())for(const r of m.revisions)if(r.sourceRevisionId===sourceRevisionId)return clone(r);return null;}
