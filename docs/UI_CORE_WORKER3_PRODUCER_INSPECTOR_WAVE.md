...
Provider credentials are deliberately excluded from Area-52 persistence. Worker 2 still publishes `ResourceCredentialStorage.SESSION_MEMORY_ONLY` for direct provider credentials, so API keys/tokens are never serialized into UI state. For the installed SillyTavern demo, Jev first uses an explicitly bound SillyTavern Connection Manager profile when one is selected; UI.Core persists only that profile ID/name and requests flow through SillyTavern's ConnectionManagerRequestService. For the common OpenRouter case, a Jev connection targeting `openrouter.ai` can also use SillyTavern's active server-side OpenRouter secret through ChatCompletionService without creating a Connection Manager profile. In both host-managed paths Area-52 receives no raw secret. Worker 2 receives `credentialRequired:false` only for the injected host-managed adapter and still requires a real provider probe before the resource becomes callable. Saved host-managed Jev locks auto-requalify on reload; direct API-key locks remain configured-only until their session credential is supplied again.
