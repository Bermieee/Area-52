...
export class ContextCompiler {
  ...
  compile({query,intent,truthResults,activeThreads=[]}){
    ...
    const keptIds=new Set([...current,...historical,...unresolvedRows,...threads].map(f=>f.id));for(const id of Object.keys(provenanceIndex))if(!keptIds.has(id))delete provenanceIndex[id];
    const dependenciesList=[...dependencies].sort(),semanticPriority=buildSemanticPriority({current,historical,unresolved:unresolvedRows,activeThreads:threads}),semanticSizing=computeSemanticSizing({current,historical,unresolved:unresolvedRows,activeThreads:threads,provenanceIndex,dependencies:dependenciesList});
    const idMaterial=[...current,...historical,...unresolvedRows].map(f=>f.id);if(threads.length)idMaterial.push(...threads.map(t=>t.id));
    return createCompiledContextPacket({id:`packet:${intent.toLowerCase()}:${hash(idMaterial.sort().join('|')||'empty')}`,query,intent,current,historical,unresolved:unresolvedRows,activeThreads:threads,provenanceIndex,dependencies:dependenciesList,semanticPriority,semanticSizing,representationEligibility:defaultRepresentationEligibility(),threadDiagnostics:{admittedThreadIds:threads.map(t=>t.threadId),staleThreadIds:threadState.staleThreadIds}});
  }
}
