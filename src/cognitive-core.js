...
export class Area52CognitiveCore {
  constructor(){
    ...
    this.settlementCore=new SettlementEngine({registry:this.registry,graph:this.graph,entityRegistry:this.entities});
    this.settlement=new SettlementBoundary({registry:this.registry,graph:this.graph,worldStateSettlement:this.settlementCore,entityRegistry:this.entities});
  ...
  invalidateEntityIdentityRevision(sourceRevisionId,options={}){const receipt=this.entities.invalidateSourceRevision(sourceRevisionId,options),invalidatedTemporalClaimIds=[];for(const ref of receipt.invalidatedIdentityRevisionRefs??[])invalidatedTemporalClaimIds.push(...this.graph.invalidateClaimsByIdentityRevision(ref));if(this.hotCognition.hasActiveChat&&(receipt.invalidatedIdentityRevisionRefs??[]).length)this.hotCognition.invalidateKnowledge({updateId:'identity-invalidated:'+String(sourceRevisionId)+':'+String(this.graph.revision),invalidatedDependencyRevisionRefs:receipt.invalidatedIdentityRevisionRefs,reason:options.reason??'IDENTITY_REVISION_INVALIDATED'});return{...receipt,invalidatedTemporalClaimIds:[...new Set(invalidatedTemporalClaimIds)].sort()};}
  worldArtifactRefsForHot(refs=[]){return (refs??[]).map(ref=>{const row=this.graph.getClaim(ref);return{ref:String(ref),identityRevisionRefs:[...(row?.identityRevisionRefs??[])],sourceRevisionRefs:[...(row?.provenance?.sourceRevisionIds??[])],dependencyRevisionRefs:[...(row?.provenance?.invalidators??[])],temporalStatus:row?.status??'CURRENT',authorityClass:row?.authorityClass??'SETTLED'};});}
  entityIdentityReadModel(options={}){return this.entities.readModel(options);}
  entityIdentityReferences(entityIds=[],options={}){return this.entities.readReferences(entityIds,options);}
  temporalStateReferences(options={}){return this.graph.readReferences(options);}
  ...
  currentWorldModel(){return{revision:this.graph.revision,current:this.graph.currentProjection(),historical:this.graph.historicalClaims().map(c=>({subjectId:c.subjectId,predicate:c.predicate,value:c.value,status:c.status,claimId:c.id,sourceRevisionIds:c.provenance?.sourceRevisionIds??[],identityRevisionRefs:c.identityRevisionRefs??[]})),unresolved:this.graph.unresolvedClaims().map(c=>({subjectId:c.subjectId,predicate:c.predicate,value:c.value,status:c.status,claimId:c.id,sourceRevisionIds:c.provenance?.sourceRevisionIds??[],identityRevisionRefs:c.identityRevisionRefs??[]}))};}
}
