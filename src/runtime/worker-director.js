  ...
  snapshot() {
    return {
      lifecycle: this.ledger.list().map((record) => ({
        ...
        startedCount: Number(record.startedCount ?? 0),
        resumeCount: Number(record.resumeCount ?? 0),
        lastSliceDurationMs: Number.isFinite(Number(record.batch?.lastSliceDurationMs)) ? Number(record.batch.lastSliceDurationMs) : null,
