...
function availability(role,p,i){const aliases=i.capabilityAliases?.[role.roleId]??i.capabilityAliases?.[role.optionId]??null,caps=Array.isArray(aliases)&&aliases.length?aliases:(role.requiredCapabilities??[]),matching=p.filter(x=>caps.every(c=>x.capabilities.includes(c))),reasons=[],explicit=i.availableCapabilities==null?null:new Set(i.availableCapabilities);
